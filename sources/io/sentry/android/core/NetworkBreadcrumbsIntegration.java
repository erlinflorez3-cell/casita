...
public final class NetworkBreadcrumbsIntegration implements Integration, Closeable {
    ...
    static class NetworkBreadcrumbConnectionDetail {
        ...
        boolean isSimilar(NetworkBreadcrumbConnectionDetail networkBreadcrumbConnectionDetail) {
            ...
            boolean z2 = DateUtils.nanosToMillis((double) Math.abs(this.timestampNanos - networkBreadcrumbConnectionDetail.timestampNanos)) < 5000.0d;
            return this.isVpn == networkBreadcrumbConnectionDetail.isVpn && this.type.equals(networkBreadcrumbConnectionDetail.type) && (z2 || iAbs <= 5) && (z2 || (((double) iAbs2) > Math.max(1000.0d, ((double) Math.abs(this.downBandwidth)) * 0.1d) ? 1 : (((double) iAbs2) == Math.max(1000.0d, ((double) Math.abs(this.downBandwidth)) * 0.1d) ? 0 : -1)) <= 0) && (z2 || (((double) iAbs3) > Math.max(1000.0d, ((double) Math.abs(this.upBandwidth)) * 0.1d) ? 1 : (((double) iAbs3) == Math.max(1000.0d, ((double) Math.abs(this.upBandwidth)) * 0.1d) ? 0 : -1)) <= 0);
        }
    }
    ...
    static final class NetworkBreadcrumbsNetworkCallback extends ConnectivityManager.NetworkCallback {
        ...
        private Breadcrumb createBreadcrumb(String str) {
            ...
            breadcrumb.setData(Constant.NOTIFICATION_DETAILS_ACTION, str);
            breadcrumb.setLevel(SentryLevel.INFO);
            return breadcrumb;
        }

        private NetworkBreadcrumbConnectionDetail getNewConnectionDetails(NetworkCapabilities networkCapabilities, NetworkCapabilities networkCapabilities2, long j2, long j3) {
            if (networkCapabilities == null) {
                return new NetworkBreadcrumbConnectionDetail(networkCapabilities2, this.buildInfoProvider, j3);
            }
            NetworkBreadcrumbConnectionDetail networkBreadcrumbConnectionDetail = new NetworkBreadcrumbConnectionDetail(networkCapabilities, this.buildInfoProvider, j2);
    ...
    /* JADX INFO: renamed from: lambda$close$0$io-sentry-android-core-NetworkBreadcrumbsIntegration, reason: not valid java name */
    /* synthetic */ void m8911xbbc7aaa6() {
