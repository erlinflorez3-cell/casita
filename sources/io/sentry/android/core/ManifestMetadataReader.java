...
final class ManifestMetadataReader {
        ...
        try {
            ...
            if (metadata != null) {
                ...
                if (sentryAndroidOptions.getSampleRate() == null) {
                    Double d2 = readDouble(metadata, logger, SAMPLE_RATE);
                    if (d2.doubleValue() != -1.0d) {
                        sentryAndroidOptions.setSampleRate(d2);
                    }
                }
                ...
                if (sentryAndroidOptions.getTracesSampleRate() == null) {
                    Double d3 = readDouble(metadata, logger, TRACES_SAMPLE_RATE);
                    if (d3.doubleValue() != -1.0d) {
                        sentryAndroidOptions.setTracesSampleRate(d3);
                    }
                }
                ...
                if (sentryAndroidOptions.getProfilesSampleRate() == null) {
                    Double d4 = readDouble(metadata, logger, PROFILES_SAMPLE_RATE);
                    if (d4.doubleValue() != -1.0d) {
                        sentryAndroidOptions.setProfilesSampleRate(d4);
                    }
                }
                ...
                long j2 = readLong(metadata, logger, IDLE_TIMEOUT, -1L);
                if (j2 != -1) {
                    sentryAndroidOptions.setIdleTimeout(Long.valueOf(j2));
                }
                ...
                if (sentryAndroidOptions.getSessionReplay().getSessionSampleRate() == null) {
                    Double d5 = readDouble(metadata, logger, REPLAYS_SESSION_SAMPLE_RATE);
                    if (d5.doubleValue() != -1.0d) {
                        sentryAndroidOptions.getSessionReplay().setSessionSampleRate(d5);
                    }
                }
                if (sentryAndroidOptions.getSessionReplay().getOnErrorSampleRate() == null) {
                    Double d6 = readDouble(metadata, logger, REPLAYS_ERROR_SAMPLE_RATE);
                    if (d6.doubleValue() != -1.0d) {
                        sentryAndroidOptions.getSessionReplay().setOnErrorSampleRate(d6);
                    }
                }
    ...
    static boolean isAutoInit(Context context, ILogger iLogger) {
        ...
        try {
            ...
            bool = readBool(metadata, iLogger, AUTO_INIT, true);
            return bool;
        } catch (Throwable th) {
    ...
    private static boolean readBool(Bundle bundle, ILogger iLogger, String str, boolean z2) {
        boolean z3 = bundle.getBoolean(str, z2);
        iLogger.log(SentryLevel.DEBUG, str + " read: " + z3, new Object[0]);
        return z3;
    }
    ...
    private static long readLong(Bundle bundle, ILogger iLogger, String str, long j2) {
