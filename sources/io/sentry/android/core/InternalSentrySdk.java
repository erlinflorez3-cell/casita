...
public final class InternalSentrySdk {
    ...
    public static SentryId captureEnvelope(byte[] bArr, boolean z2) {
        ...
        try {
            ...
            try {
                ...
                Session sessionUpdateSession = updateSession(hubAdapter, options, state, z3);
                if (sessionUpdateSession != null) {
                    arrayList.add(SentryEnvelopeItem.fromSession(serializer, sessionUpdateSession));
                    deleteCurrentSessionFile(options, (z2 && hubAdapter.getOptions().getMainThreadChecker().isMainThread()) ? false : true);
                    if (z2) {
    ...
    private static void deleteCurrentSessionFile(final SentryOptions sentryOptions, boolean z2) {
        if (z2) {
    ...
    public static IScope getCurrentScope() {
        ...
        HubAdapter.getInstance().configureScope(new ScopeCallback() { // from class: io.sentry.android.core.InternalSentrySdk$$ExternalSyntheticLambda0
            ...
            public final void run(IScope iScope) {
                atomicReference.set(iScope.m8872clone());
            }
        });
        return (IScope) atomicReference.get();
    }

    static /* synthetic */ void lambda$updateSession$2(Session.State state, boolean z2, AtomicReference atomicReference, SentryOptions sentryOptions, IScope iScope) {
        ...
        } else if (session.update(state, null, z2, null)) {
        ...
        try {
            ...
            if (user.getId() == null) {
                try {
                    ...
                    logger.log(SentryLevel.ERROR, "Could not retrieve installation ID", e2);
                }
            }
            ...
            mapObjectWriter.name(InAppMessageBase.EXTRAS).value(logger, iScope.getExtras());
    ...
    private static Session updateSession(IHub iHub, final SentryOptions sentryOptions, final Session.State state, final boolean z2) {
        final AtomicReference atomicReference = new AtomicReference();
        iHub.configureScope(new ScopeCallback() { // from class: io.sentry.android.core.InternalSentrySdk$$ExternalSyntheticLambda2
            @Override // io.sentry.ScopeCallback
            public final void run(IScope iScope) {
                InternalSentrySdk.lambda$updateSession$2(state, z2, atomicReference, sentryOptions, iScope);
            }
        });
