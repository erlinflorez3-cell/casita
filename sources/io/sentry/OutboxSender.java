...
public final class OutboxSender extends DirectoryProcessor implements IEnvelopeSender {
    ...
    public OutboxSender(IHub iHub, IEnvelopeReader iEnvelopeReader, ISerializer iSerializer, ILogger iLogger, long j2, int i2) {
        super(iHub, iLogger, j2, i2);
        ...
        if (traceContext != null && (sampleRate = traceContext.getSampleRate()) != null) {
            try {
                ...
                if (SampleRateUtils.isValidTracesSampleRate(dValueOf, false)) {
                    return new TracesSamplingDecision(true, dValueOf);
                }
    ...
    private void logEnvelopeItemNull(SentryEnvelopeItem sentryEnvelopeItem, int i2) {
    ...
    private void logUnexpectedEventId(SentryEnvelope sentryEnvelope, SentryId sentryId, int i2) {
        this.logger.log(SentryLevel.ERROR, "Item %d of has a different event id (%s) to the envelope header (%s)", Integer.valueOf(i2), sentryEnvelope.getHeader().getEventId(), sentryId);
    }
        ...
        for (SentryEnvelopeItem sentryEnvelopeItem : sentryEnvelope.getItems()) {
            ...
            if (sentryEnvelopeItem.getHeader() == null) {
                    ...
                    try {
                        ...
                        if (sentryEvent == null) {
                            ...
                            if (sentryEnvelope.getHeader().getEventId() == null || sentryEnvelope.getHeader().getEventId().equals(sentryEvent.getEventId())) {
                                ...
                                logUnexpectedEventId(sentryEnvelope, sentryEvent.getEventId(), i2);
                                bufferedReader.close();
                            }
                        }
                        bufferedReader.close();
                ...
                if (SentryItemType.Transaction.equals(sentryEnvelopeItem.getHeader().getType())) {
                    ...
                    this.logger.log(SentryLevel.DEBUG, "%s item %d is being captured.", sentryEnvelopeItem.getHeader().getType().getItemType(), Integer.valueOf(i2));
                    if (!waitFlush(hint)) {
    ...
    /* JADX INFO: renamed from: lambda$processFile$0$io-sentry-OutboxSender, reason: not valid java name */
    /* synthetic */ void m8873lambda$processFile$0$iosentryOutboxSender(File file, Retryable retryable) {
            ...
            try {
                ...
                try {
                    ...
                    sentryConsumer = new HintUtils.SentryConsumer() { // from class: io.sentry.OutboxSender$$ExternalSyntheticLambda1
                        @Override // io.sentry.util.HintUtils.SentryConsumer
                        public final void accept(Object obj) {
                            this.f$0.m8873lambda$processFile$0$iosentryOutboxSender(file, (Retryable) obj);
                        }
                    };
                } catch (Throwable th) {
                    try {
