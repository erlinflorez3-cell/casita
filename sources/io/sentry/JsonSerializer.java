...
public final class JsonSerializer implements ISerializer {
    ...
    private String serializeToString(Object obj, boolean z2) throws IOException {
    ...
    public <T> T deserialize(Reader reader, Class<T> cls) {
        try {
            ...
            this.options.getLogger().log(SentryLevel.ERROR, "Error when deserializing", e2);
            return null;
        }
    }
    ...
    public <T, R> T deserializeCollection(Reader reader, Class<T> cls, JsonDeserializer<R> jsonDeserializer) {
        try {
            ...
            try {
                ...
                if (jsonDeserializer == null) {
                    T t3 = (T) jsonObjectReader.nextObjectOrNull();
                    jsonObjectReader.close();
                    return t3;
                }
                T t4 = (T) jsonObjectReader.nextListOrNull(this.options.getLogger(), jsonDeserializer);
                jsonObjectReader.close();
                return t4;
            } finally {
            }
    ...
    public SentryEnvelope deserializeEnvelope(InputStream inputStream) {
        ...
        try {
            ...
            this.options.getLogger().log(SentryLevel.ERROR, "Error deserializing envelope.", e2);
            return null;
        }
    }
        ...
        try {
            ...
            for (SentryEnvelopeItem sentryEnvelopeItem : sentryEnvelope.getItems()) {
                try {
                    ...
                    this.options.getLogger().log(SentryLevel.ERROR, "Failed to create envelope item. Dropping it.", e2);
                }
            }
        } finally {
            bufferedWriter.flush();
    ...
    public <T> void serialize(T t2, Writer writer) throws IOException {
        ...
        if (this.options.getLogger().isEnabled(SentryLevel.DEBUG)) {
            this.options.getLogger().log(SentryLevel.DEBUG, "Serializing object: %s", serializeToString(t2, this.options.isEnablePrettySerializationOutput()));
        }
        new JsonObjectWriter(writer, this.options.getMaxDepth()).value(this.options.getLogger(), (Object) t2);
        writer.flush();
    }
}
