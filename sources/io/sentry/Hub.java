...
public final class Hub implements IHub, MetricsApi.IMetricsInterface {
    ...
    private IScope buildLocalScope(IScope iScope, ScopeCallback scopeCallback) {
        if (scopeCallback != null) {
            try {
                IScope iScopeM8874clone = iScope.m8874clone();
                scopeCallback.run(iScopeM8874clone);
                return iScopeM8874clone;
            } catch (Throwable th) {
    ...
    private SentryId captureMessageInternal(String str, SentryLevel sentryLevel, ScopeCallback scopeCallback) {
        ...
        if (!isEnabled()) {
            ...
            try {
                Stack.StackItem stackItemPeek = this.stack.peek();
                sentryIdCaptureMessage = stackItemPeek.getClient().captureMessage(str, sentryLevel, buildLocalScope(stackItemPeek.getScope(), scopeCallback));
            } catch (Throwable th) {
                this.options.getLogger().log(SentryLevel.ERROR, "Error while capturing message: " + str, th);
            }
        }
        this.lastEventId = sentryIdCaptureMessage;
    ...
    private ITransaction createTransaction(TransactionContext transactionContext, TransactionOptions transactionOptions) {
        ...
        if (!isEnabled()) {
            ...
            TracesSamplingDecision tracesSamplingDecisionSample = this.tracesSampler.sample(new SamplingContext(transactionContext, transactionOptions.getCustomSamplingContext()));
            transactionContext.setSamplingDecision(tracesSamplingDecisionSample);
            sentryTracer = new SentryTracer(transactionContext, this, transactionOptions, this.transactionPerformanceCollector);
    ...
    public void bindClient(ISentryClient iSentryClient) {
        ...
        if (iSentryClient != null) {
            ...
            stackItemPeek.setClient(iSentryClient);
        } else {
            this.options.getLogger().log(SentryLevel.DEBUG, "NoOp client bound to scope.", new Object[0]);
            stackItemPeek.setClient(NoOpSentryClient.getInstance());
        }
    }
    ...
    public SentryId captureCheckIn(CheckIn checkIn) {
        ...
        if (isEnabled()) {
            try {
                Stack.StackItem stackItemPeek = this.stack.peek();
                sentryIdCaptureCheckIn = stackItemPeek.getClient().captureCheckIn(checkIn, stackItemPeek.getScope(), null);
            } catch (Throwable th) {
        ...
        this.lastEventId = sentryIdCaptureCheckIn;
    ...
    public SentryId captureReplay(SentryReplayEvent sentryReplayEvent, Hint hint) {
        SentryId sentryIdCaptureReplayEvent = SentryId.EMPTY_ID;
        if (!isEnabled()) {
            this.options.getLogger().log(SentryLevel.WARNING, "Instance is disabled and this 'captureReplay' call is a no-op.", new Object[0]);
            return sentryIdCaptureReplayEvent;
        }
        try {
            Stack.StackItem stackItemPeek = this.stack.peek();
            sentryIdCaptureReplayEvent = stackItemPeek.getClient().captureReplayEvent(sentryReplayEvent, stackItemPeek.getScope(), hint);
            return sentryIdCaptureReplayEvent;
        } catch (Throwable th) {
    ...
    public SentryId captureTransaction(SentryTransaction sentryTransaction, TraceContext traceContext, Hint hint, ProfilingTraceData profilingTraceData) {
        ...
        SentryId sentryIdCaptureTransaction = SentryId.EMPTY_ID;
        if (!isEnabled()) {
        ...
        if (Boolean.TRUE.equals(Boolean.valueOf(sentryTransaction.isSampled()))) {
            try {
                Stack.StackItem stackItemPeek = this.stack.peek();
                sentryIdCaptureTransaction = stackItemPeek.getClient().captureTransaction(sentryTransaction, traceContext, stackItemPeek.getScope(), hint, profilingTraceData);
                return sentryIdCaptureTransaction;
            } catch (Throwable th) {
            ...
            for (Integration integration : this.options.getIntegrations()) {
                if (integration instanceof Closeable) {
                    try {
                        ...
                        this.options.getLogger().log(SentryLevel.WARNING, "Failed to close the integration {}.", integration, e2);
                    }
                }
            }
            ...
            if (z2) {
                executorService.submit(new Runnable() { // from class: io.sentry.Hub$$ExternalSyntheticLambda1
                    ...
                    public final void run() {
                        this.f$0.m8867lambda$close$1$iosentryHub(executorService);
                    }
                });
            } else {
                executorService.close(this.options.getShutdownTimeoutMillis());
            }
            this.stack.peek().getClient().close(z2);
        } catch (Throwable th) {
    ...
    public TransactionContext continueTrace(String str, List<String> list) {
        ...
        configureScope(new ScopeCallback() { // from class: io.sentry.Hub$$ExternalSyntheticLambda2
            @Override // io.sentry.ScopeCallback
            public final void run(IScope iScope) {
                iScope.setPropagationContext(propagationContextFromHeaders);
            }
        });
    ...
    public void flush(long j2) {
        ...
        try {
            this.stack.peek().getClient().flush(j2);
        } catch (Throwable th) {
    ...
    public Map<String, String> getDefaultTagsForMetrics() {
        ...
        if (transactionName != null) {
            map.put("transaction", transactionName);
        }
    ...
    /* JADX INFO: renamed from: lambda$close$1$io-sentry-Hub */
    /* synthetic */ void m8867lambda$close$1$iosentryHub(ISentryExecutorService iSentryExecutorService) {
    ...
    public void pushScope() {
        ...
        Stack.StackItem stackItemPeek = this.stack.peek();
        this.stack.push(new Stack.StackItem(this.options, stackItemPeek.getClient(), stackItemPeek.getScope().m8874clone()));
    }
