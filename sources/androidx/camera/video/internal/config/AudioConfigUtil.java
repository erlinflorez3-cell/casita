...
public final class AudioConfigUtil {
    ...
    static int selectSampleRateOrNearestSupported(Range<Integer> range, int i2, int i3, final int i4) {
        ArrayList arrayList = null;
        int i5 = 0;
        int iIntValue = i4;
        while (true) {
            if (range.contains(Integer.valueOf(iIntValue))) {
                if (AudioSource.isSettingsSupported(iIntValue, i2, i3)) {
                    return iIntValue;
                }
                Logger.d(TAG, "Sample rate " + iIntValue + "Hz is not supported by audio source with channel count " + i2 + " and source format " + i3);
            } else {
                Logger.d(TAG, "Sample rate " + iIntValue + "Hz is not in target range " + range);
            }
            if (arrayList == null) {
                Logger.d(TAG, "Trying common sample rates in proximity order to target " + i4 + "Hz");
                ...
                Collections.sort(arrayList, new Comparator() { // from class: androidx.camera.video.internal.config.AudioConfigUtil$$ExternalSyntheticLambda0
                    ...
                    public final int compare(Object obj, Object obj2) {
                        return AudioConfigUtil.lambda$selectSampleRateOrNearestSupported$0(i4, (Integer) obj, (Integer) obj2);
                    }
                });
            }
            if (i5 < arrayList.size()) {
                iIntValue = ((Integer) arrayList.get(i5)).intValue();
                i5++;
            } else {
    ...
    static /* synthetic */ int lambda$selectSampleRateOrNearestSupported$0(int i2, Integer num, Integer num2) {
        float fSignum;
    ...
    static int scaleAndClampBitrate(int i2, int i3, int i4, int i5, int i6, Range<Integer> range) {
        String str;
        int iDoubleValue = (int) (((double) i2) * new Rational(i3, i4).doubleValue() * new Rational(i5, i6).doubleValue());
        if (!Logger.isDebugEnabled(TAG)) {
            str = "";
        } else {
            str = String.format("Base Bitrate(%dbps) * Channel Count Ratio(%d / %d) * Sample Rate Ratio(%d / %d) = %d", Integer.valueOf(i2), Integer.valueOf(i3), Integer.valueOf(i4), Integer.valueOf(i5), Integer.valueOf(i6), Integer.valueOf(iDoubleValue));
        }
