...
class CameraStateMachine {
    ...
    public void updateState(CameraInternal.State state, CameraState.StateError stateError) {
        CameraState cameraStateCreate;
        switch (AnonymousClass1.$SwitchMap$androidx$camera$core$impl$CameraInternal$State[state.ordinal()]) {
            case 1:
                cameraStateCreate = onCameraPendingOpen();
                break;
            case 2:
                cameraStateCreate = CameraState.create(CameraState.Type.OPENING, stateError);
                break;
            case 3:
            case 4:
                cameraStateCreate = CameraState.create(CameraState.Type.OPEN, stateError);
                break;
            case 5:
            case 6:
                cameraStateCreate = CameraState.create(CameraState.Type.CLOSING, stateError);
                break;
            case 7:
            case 8:
                cameraStateCreate = CameraState.create(CameraState.Type.CLOSED, stateError);
                break;
            default:
                throw new IllegalStateException("Unknown internal camera state: " + state);
        }
        Logger.d(TAG, "New public camera state " + cameraStateCreate + " from " + state + " and " + stateError);
        if (Objects.equals(this.mCameraStates.getValue(), cameraStateCreate)) {
            return;
        }
        Logger.d(TAG, "Publishing new public camera state " + cameraStateCreate);
        this.mCameraStates.postValue(cameraStateCreate);
    }
