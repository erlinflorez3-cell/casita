...
public abstract class BaseAgreementSpi extends KeyAgreementSpi {
    ...
    static {
        ...
        map2.put(PKCSObjectIdentifiers.id_hmacWithSHA384.getId(), Integers.valueOf(BitmapCounterConfig.DEFAULT_MAX_BITMAP_COUNT));
    ...
    protected SecretKey engineGenerateSecret(String str) throws NoSuchAlgorithmException {
        ...
        if (derivationFunction != null) {
            ...
            if (derivationFunction instanceof DHKEKGenerator) {
                try {
                    this.kdf.init(new DHKDFParameters(new ASN1ObjectIdentifier(id), keySize, bArrBigIntToBytes, this.ukmParameters));
                } catch (IllegalArgumentException unused) {
                    throw new NoSuchAlgorithmException("no OID for algorithm: " + id);
                }
            } else {
                this.kdf.init(new KDFParameters(bArrBigIntToBytes, this.ukmParameters));
            }
            ...
            System.arraycopy(bArrBigIntToBytes, 0, bArr2, 0, i3);
            bArrBigIntToBytes = bArr2;
        }
        if (des.containsKey(id)) {
            DESParameters.setOddParity(bArrBigIntToBytes);
        }
        return new SecretKeySpec(bArrBigIntToBytes, getAlgorithm(str));
    }
