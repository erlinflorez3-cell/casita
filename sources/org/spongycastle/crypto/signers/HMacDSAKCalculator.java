...
public class HMacDSAKCalculator implements DSAKCalculator {
    ...
    private BigInteger bitsToInt(byte[] bArr) {
        ...
        return bArr.length * 8 > this.f27980n.bitLength() ? bigInteger.shiftRight((bArr.length * 8) - this.f27980n.bitLength()) : bigInteger;
    }

    @Override // org.spongycastle.crypto.signers.DSAKCalculator
    public void init(BigInteger bigInteger, BigInteger bigInteger2, byte[] bArr) {
        this.f27980n = bigInteger;
    ...
    public BigInteger nextK() {
        int iBitLength = (this.f27980n.bitLength() + 7) / 8;
        ...
        while (true) {
            ...
            while (i2 < iBitLength) {
                ...
                this.hMac.doFinal(this.V, 0);
                int iMin = Math.min(iBitLength - i2, this.V.length);
                System.arraycopy(this.V, 0, bArr, i2, iMin);
                i2 += iMin;
            }
            BigInteger bigIntegerBitsToInt = bitsToInt(bArr);
            if (bigIntegerBitsToInt.compareTo(ZERO) > 0 && bigIntegerBitsToInt.compareTo(this.f27980n) < 0) {
