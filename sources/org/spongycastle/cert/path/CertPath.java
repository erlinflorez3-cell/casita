...
public class CertPath {
    ...
    public CertPathValidationResult evaluate(CertPathValidation[] certPathValidationArr) {
        ...
        for (int i2 = 0; i2 != certPathValidationArr.length; i2++) {
            int length = this.certificates.length - 1;
            while (length >= 0) {
                try {
                    certPathValidationContext.setIsEndEntity(length == 0);
                    certPathValidationArr[i2].validate(certPathValidationContext, this.certificates[length]);
        ...
        for (int i2 = 0; i2 != certPathValidationArr.length; i2++) {
            ...
            while (length >= 0) {
                try {
                    ...
                    return new CertPathValidationResult(certPathValidationContext, length, i2, e2);
                }
            }
        }
