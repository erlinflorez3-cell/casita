...
public final class Blake3 {
    ...
    private static final class ChunkState {
        ...
        private ChunkState(int[] iArr, long j2, int i2) {
            this.block = new byte[64];
            this.chainingValue = iArr;
            this.chunkCounter = j2;
            this.flags = i2;
        }
        ...
        public Output output() {
            return new Output(this.chainingValue, Blake3.unpackInts(this.block, 16), this.chunkCounter, this.blockLength, ((-1) - (((-1) - this.flags) & ((-1) - startFlag()))) | 2);
        }
        ...
        public void update(byte[] bArr, int i2, int i3) {
            while (i3 > 0) {
                if (this.blockLength == 64) {
                    this.chainingValue = Arrays.copyOf(Blake3.compress(this.chainingValue, Blake3.unpackInts(this.block, 16), 64, this.chunkCounter, (-1) - (((-1) - this.flags) & ((-1) - startFlag()))), 8);
                    this.blocksCompressed++;
                    this.blockLength = 0;
                    Arrays.fill(this.block, (byte) 0);
                }
                int iMin = Math.min(64 - this.blockLength, i3);
                System.arraycopy(bArr, i2, this.block, this.blockLength, iMin);
    ...
    private static final class EngineState {
        ...
        private EngineState(int[] iArr, int i2) {
            this.cvStack = new int[54][];
            this.key = iArr;
            this.flags = i2;
            this.state = new ChunkState(iArr, 0L, i2);
        }

        private void addChunkCV(int[] iArr, long j2) {
            while ((1 + j2) - (1 | j2) == 0) {
        ...
        public void inputData(byte[] bArr, int i2, int i3) {
            while (i3 > 0) {
                if (this.state.length() == 1024) {
                    ...
                    long j2 = this.state.chunkCounter + 1;
                    addChunkCV(iArrChainingValue, j2);
                    this.state = new ChunkState(this.key, j2, this.flags);
                }
                int iMin = Math.min(1024 - this.state.length(), i3);
                this.state.update(bArr, i2, iMin);
                i2 += iMin;
                i3 -= iMin;
            }
        }
        ...
        /* JADX INFO: Access modifiers changed from: private */
        public void outputHash(byte[] bArr, int i2, int i3) {
            Output output = this.state.output();
            int i4 = this.stackLen;
            while (true) {
                int i5 = i4 - 1;
                if (i4 <= 0) {
                    output.rootOutputBytes(bArr, i2, i3);
                    return;
                } else {
                    output = Blake3.parentOutput(this.cvStack[i5], output.chainingValue(), this.key, this.flags);
                    i4 = i5;
                }
            }
        }
    ...
    private static final class Output {
        ...
        private Output(int[] iArr, int[] iArr2, long j2, int i2, int i3) {
        ...
        /* JADX INFO: Access modifiers changed from: private */
        public void rootOutputBytes(byte[] bArr, int i2, int i3) {
            int i4 = i2;
            int i5 = i3;
            int i6 = 0;
            while (i5 > 0) {
                int iMin = Math.min(64, i5);
                i5 -= iMin;
                int i7 = i6 + 1;
                int[] iArrCompress = Blake3.compress(this.inputChainingValue, this.blockWords, this.blockLength, i6, this.flags | 8);
                int i8 = 0;
                while (iMin > 0) {
                    int iMin2 = Math.min(4, iMin);
                    Blake3.packInt(iArrCompress[i8], bArr, i4, iMin2);
                    i4 += iMin2;
                    iMin -= iMin2;
                    i8++;
                }
    ...
    private Blake3(int[] iArr, int i2) {
        this.engineState = new EngineState(iArr, i2);
    }

    private static void checkBufferArgs(byte[] bArr, int i2, int i3) {
        ...
        if (i2 > length - i3) {
            throw new IndexOutOfBoundsException("Offset " + i2 + " and length " + i3 + " out of bounds with buffer length " + length);
        }
    }

    /* JADX INFO: Access modifiers changed from: private */
    public static int[] compress(int[] iArr, int[] iArr2, int i2, long j2, int i3) {
        int[] iArrCopyOf = Arrays.copyOf(iArr, 16);
        System.arraycopy(IV, 0, iArrCopyOf, 8, 4);
        iArrCopyOf[12] = (int) j2;
        iArrCopyOf[13] = (int) (j2 >> 32);
        iArrCopyOf[14] = i2;
        iArrCopyOf[15] = i3;
        for (int i4 = 0; i4 < 7; i4++) {
            round(iArrCopyOf, iArr2, MSG_SCHEDULE[i4]);
        }
        for (int i5 = 0; i5 < iArrCopyOf.length / 2; i5++) {
            int i6 = i5 + 8;
            iArrCopyOf[i5] = iArrCopyOf[i5] ^ iArrCopyOf[i6];
            iArrCopyOf[i6] = iArrCopyOf[i6] ^ iArr[i5];
        }
        return iArrCopyOf;
    }

    private static void g(int[] iArr, int i2, int i3, int i4, int i5, int i6, int i7) {
        int i8 = iArr[i2] + iArr[i3] + i6;
        iArr[i2] = i8;
        int iRotateRight = Integer.rotateRight(iArr[i5] ^ i8, 16);
        iArr[i5] = iRotateRight;
        int i9 = iArr[i4] + iRotateRight;
        iArr[i4] = i9;
        int iRotateRight2 = Integer.rotateRight(iArr[i3] ^ i9, 12);
        iArr[i3] = iRotateRight2;
        int i10 = iArr[i2] + iRotateRight2 + i7;
        iArr[i2] = i10;
        int iRotateRight3 = Integer.rotateRight(iArr[i5] ^ i10, 8);
        iArr[i5] = iRotateRight3;
        int i11 = iArr[i4] + iRotateRight3;
        iArr[i4] = i11;
        iArr[i3] = Integer.rotateRight(iArr[i3] ^ i11, 7);
    }
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public static void packInt(int i2, byte[] bArr, int i3, int i4) {
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public static int[] parentChainingValue(int[] iArr, int[] iArr2, int[] iArr3, int i2) {
        return parentOutput(iArr, iArr2, iArr3, i2).chainingValue();
    }

    /* JADX INFO: Access modifiers changed from: private */
    public static Output parentOutput(int[] iArr, int[] iArr2, int[] iArr3, int i2) {
        int[] iArrCopyOf = Arrays.copyOf(iArr, 16);
        System.arraycopy(iArr2, 0, iArrCopyOf, 8, 8);
        return new Output((int[]) iArr3.clone(), iArrCopyOf, 0L, 64, (-1) - (((-1) - i2) & ((-1) - 4)));
    }
    ...
    private static int unpackInt(byte[] bArr, int i2) {
        int i3 = bArr[i2] & 255;
        int i4 = ((-1) - (((-1) - bArr[i2 + 1]) | ((-1) - 255))) << 8;
        return (((-1) - (((-1) - bArr[i2 + 3]) | ((-1) - 255))) << 24) | ((i3 + i4) - (i3 & i4)) | (((-1) - (((-1) - bArr[i2 + 2]) | ((-1) - 255))) << 16);
    }

    /* JADX INFO: Access modifiers changed from: private */
    public static int[] unpackInts(byte[] bArr, int i2) {
        int[] iArr = new int[i2];
        int i3 = 0;
        int i4 = 0;
        while (i3 < i2) {
            iArr[i3] = unpackInt(bArr, i4);
            i3++;
            i4 += 4;
        }
        return iArr;
    ...
    public Blake3 doFinalize(byte[] bArr, int i2, int i3) {
        checkBufferArgs(bArr, i2, i3);
        this.engineState.outputHash(bArr, i2, i3);
        return this;
    }

    public byte[] doFinalize(int i2) {
        if (i2 < 0) {
    ...
    public Blake3 update(byte[] bArr, int i2, int i3) {
        checkBufferArgs(bArr, i2, i3);
        this.engineState.inputData(bArr, i2, i3);
        return this;
    }
}
