...
public abstract class VirtualView extends ReactViewGroup {
    ...
    /* JADX INFO: renamed from: com.horcrux.svg.VirtualView$1 */
    ...
    abstract void draw(Canvas canvas, Paint paint, float f2);
    ...
    protected void onMeasure(int i2, int i3) {
        setMeasuredDimension(this.mClientRect != null ? (int) Math.ceil(r0.width()) : getDefaultSize(getSuggestedMinimumWidth(), i2), this.mClientRect != null ? (int) Math.ceil(r0.height()) : getDefaultSize(getSuggestedMinimumHeight(), i3));
    }

    double relativeOn(SVGLength sVGLength, float f2) {
        SVGLength.UnitType unitType = sVGLength.unit;
        return unitType == SVGLength.UnitType.NUMBER ? sVGLength.value * ((double) this.mScale) : unitType == SVGLength.UnitType.PERCENTAGE ? (sVGLength.value / 100.0d) * ((double) f2) : fromRelativeFast(sVGLength);
    }

    double relativeOnFraction(SVGLength sVGLength, float f2) {
        double d2;
        SVGLength.UnitType unitType = sVGLength.unit;
    ...
    void restoreCanvas(Canvas canvas, int i2) {
