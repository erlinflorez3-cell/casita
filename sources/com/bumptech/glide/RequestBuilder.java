...
public class RequestBuilder<TranscodeType> extends BaseRequestOptions<RequestBuilder<TranscodeType>> implements Cloneable, ModelTypes<RequestBuilder<TranscodeType>> {
    ...
    private Request buildRequestRecursive(Object obj, Target<TranscodeType> target, RequestListener<TranscodeType> requestListener, RequestCoordinator requestCoordinator, TransitionOptions<?, ? super TranscodeType> transitionOptions, Priority priority, int i2, int i3, BaseRequestOptions<?> baseRequestOptions, Executor executor) {
        ...
        Request requestBuildThumbnailRequestRecursive = buildThumbnailRequestRecursive(obj, target, requestListener, errorRequestCoordinator2, transitionOptions, priority, i2, i3, baseRequestOptions, executor);
    ...
    private <Y extends Target<TranscodeType>> Y into(Y y2, RequestListener<TranscodeType> requestListener, BaseRequestOptions<?> baseRequestOptions, Executor executor) {
    ...
    public RequestBuilder<TranscodeType> mo7079clone() {
        RequestBuilder<TranscodeType> requestBuilder = (RequestBuilder) super.mo7079clone();
        requestBuilder.transitionOptions = requestBuilder.transitionOptions.m7080clone();
    ...
    <Y extends Target<TranscodeType>> Y into(Y y2, RequestListener<TranscodeType> requestListener, Executor executor) {
        return (Y) into(y2, requestListener, this, executor);
    }

    public ViewTarget<ImageView, TranscodeType> into(ImageView imageView) {
        BaseRequestOptions baseRequestOptionsOptionalCenterInside;
        Util.assertMainThread();
        Preconditions.checkNotNull(imageView);
        if (!isTransformationSet() && isTransformationAllowed() && imageView.getScaleType() != null) {
            switch (AnonymousClass1.$SwitchMap$android$widget$ImageView$ScaleType[imageView.getScaleType().ordinal()]) {
                case 1:
                    baseRequestOptionsOptionalCenterInside = mo7079clone().optionalCenterCrop();
                    break;
                case 2:
                    baseRequestOptionsOptionalCenterInside = mo7079clone().optionalCenterInside();
                    break;
                case 3:
                case 4:
                case 5:
                    baseRequestOptionsOptionalCenterInside = mo7079clone().optionalFitCenter();
                    break;
                case 6:
                    baseRequestOptionsOptionalCenterInside = mo7079clone().optionalCenterInside();
                    break;
                default:
                    baseRequestOptionsOptionalCenterInside = this;
                    break;
            }
        } else {
            baseRequestOptionsOptionalCenterInside = this;
        }
        return (ViewTarget) into(this.glideContext.buildImageViewTarget(imageView, this.transcodeClass), null, baseRequestOptionsOptionalCenterInside, Executors.mainThreadExecutor());
    }
