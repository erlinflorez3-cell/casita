...
public class LruBitmapPool implements BitmapPool {
    ...
    private synchronized void trimToSize(long j2) {
        while (this.currentSize > j2) {
            ...
            if (bitmapRemoveLast == null) {
                ...
                this.evictions++;
                if (Log.isLoggable(TAG, 3)) {
                    String str = "Evicting bitmap=" + this.strategy.logBitmap(bitmapRemoveLast);
                }
                dump();
    ...
    public synchronized void put(Bitmap bitmap) {
        ...
        if (!bitmap.isMutable() || this.strategy.getSize(bitmap) > this.maxSize || !this.allowedConfigs.contains(bitmap.getConfig())) {
            if (Log.isLoggable(TAG, 2)) {
                String str = "Reject bitmap from pool, bitmap: " + this.strategy.logBitmap(bitmap) + ", is mutable: " + bitmap.isMutable() + ", is allowed config: " + this.allowedConfigs.contains(bitmap.getConfig());
            }
            bitmap.recycle();
            return;
        }
        ...
        if (Log.isLoggable(TAG, 2)) {
            String str2 = "Put bitmap in pool=" + this.strategy.logBitmap(bitmap);
        }
        dump();
        evict();
    }
