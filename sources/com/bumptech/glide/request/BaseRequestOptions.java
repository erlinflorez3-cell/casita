...
public abstract class BaseRequestOptions<T extends BaseRequestOptions<T>> implements Cloneable {
    ...
    public T apply(BaseRequestOptions<?> baseRequestOptions) {
        if (this.isAutoCloneEnabled) {
            return (T) mo7079clone().apply(baseRequestOptions);
        }
        ...
        if (!this.isTransformationAllowed) {
            ...
            this.fields = (i4 - 133121) - (i4 | (-133121));
            this.isScaleOnlyOrNoTransform = true;
        }
        this.fields = (-1) - (((-1) - this.fields) & ((-1) - baseRequestOptions.fields));
    ...
    public T decode(Class<?> cls) {
        if (this.isAutoCloneEnabled) {
            return (T) mo7079clone().decode(cls);
        }
        this.resourceClass = (Class) Preconditions.checkNotNull(cls);
        int i2 = this.fields;
        this.fields = (i2 + 4096) - (i2 & 4096);
        return (T) selfOrThrowIfLocked();
    }
    ...
    public T diskCacheStrategy(DiskCacheStrategy diskCacheStrategy) {
        if (this.isAutoCloneEnabled) {
            return (T) mo7079clone().diskCacheStrategy(diskCacheStrategy);
        }
        this.diskCacheStrategy = (DiskCacheStrategy) Preconditions.checkNotNull(diskCacheStrategy);
        this.fields = (-1) - (((-1) - this.fields) & ((-1) - 4));
        return (T) selfOrThrowIfLocked();
    }
    ...
    public T dontTransform() {
        if (this.isAutoCloneEnabled) {
            return (T) mo7079clone().dontTransform();
        }
        this.transformations.clear();
        int i2 = this.fields;
        this.isTransformationRequired = false;
        int i3 = (i2 - 133121) - (i2 | (-133121));
        this.isTransformationAllowed = false;
        this.fields = (i3 + 65536) - (i3 & 65536);
        this.isScaleOnlyOrNoTransform = true;
        return (T) selfOrThrowIfLocked();
    }
    ...
    public T error(int i2) {
        if (this.isAutoCloneEnabled) {
            return (T) mo7079clone().error(i2);
        }
        this.errorId = i2;
        int i3 = this.fields;
        int i4 = (i3 + 32) - (i3 & 32);
        this.errorPlaceholder = null;
        this.fields = (i4 - 17) - (i4 | (-17));
        return (T) selfOrThrowIfLocked();
    }

    public T error(Drawable drawable) {
        if (this.isAutoCloneEnabled) {
            return (T) mo7079clone().error(drawable);
        }
        this.errorPlaceholder = drawable;
        int i2 = this.fields;
        this.errorId = 0;
        this.fields = ((i2 + 16) - (i2 & 16)) & (-33);
        return (T) selfOrThrowIfLocked();
    }

    public T fallback(int i2) {
        if (this.isAutoCloneEnabled) {
            return (T) mo7079clone().fallback(i2);
        }
        this.fallbackId = i2;
        int i3 = this.fields | 16384;
        this.fallbackDrawable = null;
        this.fields = (i3 - 8193) - (i3 | (-8193));
        return (T) selfOrThrowIfLocked();
    }

    public T fallback(Drawable drawable) {
        if (this.isAutoCloneEnabled) {
            return (T) mo7079clone().fallback(drawable);
        }
        this.fallbackDrawable = drawable;
        int i2 = this.fields | 8192;
        this.fallbackId = 0;
        this.fields = (i2 - 16385) - (i2 | (-16385));
        return (T) selfOrThrowIfLocked();
    }
    ...
    public T onlyRetrieveFromCache(boolean z2) {
        if (this.isAutoCloneEnabled) {
            return (T) mo7079clone().onlyRetrieveFromCache(z2);
        }
        this.onlyRetrieveFromCache = z2;
        this.fields = (-1) - (((-1) - this.fields) & ((-1) - 524288));
        return (T) selfOrThrowIfLocked();
    }
    ...
    final T optionalTransform(DownsampleStrategy downsampleStrategy, Transformation<Bitmap> transformation) {
        if (this.isAutoCloneEnabled) {
            return (T) mo7079clone().optionalTransform(downsampleStrategy, transformation);
        }
    ...
    public T override(int i2, int i3) {
        if (this.isAutoCloneEnabled) {
            return (T) mo7079clone().override(i2, i3);
        }
    ...
    public T placeholder(int i2) {
        if (this.isAutoCloneEnabled) {
            return (T) mo7079clone().placeholder(i2);
        }
        this.placeholderId = i2;
        int i3 = (-1) - (((-1) - this.fields) & ((-1) - 128));
        this.placeholderDrawable = null;
        this.fields = (-1) - (((-1) - i3) | ((-1) - (-65)));
        return (T) selfOrThrowIfLocked();
    }

    public T placeholder(Drawable drawable) {
        if (this.isAutoCloneEnabled) {
            return (T) mo7079clone().placeholder(drawable);
        }
        this.placeholderDrawable = drawable;
        int i2 = this.fields;
        this.placeholderId = 0;
        this.fields = (-1) - (((-1) - ((i2 + 64) - (i2 & 64))) | ((-1) - (-129)));
        return (T) selfOrThrowIfLocked();
    }

    public T priority(Priority priority) {
        if (this.isAutoCloneEnabled) {
            return (T) mo7079clone().priority(priority);
        }
        this.priority = (Priority) Preconditions.checkNotNull(priority);
        this.fields = (-1) - (((-1) - this.fields) & ((-1) - 8));
        return (T) selfOrThrowIfLocked();
    }

    public <Y> T set(Option<Y> option, Y y2) {
        if (this.isAutoCloneEnabled) {
            return (T) mo7079clone().set(option, y2);
        }
    ...
    public T signature(Key key) {
        if (this.isAutoCloneEnabled) {
            return (T) mo7079clone().signature(key);
        }
    ...
    public T sizeMultiplier(float f2) {
        if (this.isAutoCloneEnabled) {
            return (T) mo7079clone().sizeMultiplier(f2);
        }
        if (f2 < 0.0f || f2 > 1.0f) {
            throw new IllegalArgumentException("sizeMultiplier must be between 0 and 1");
        }
        this.sizeMultiplier = f2;
        this.fields = (-1) - (((-1) - this.fields) & ((-1) - 2));
        return (T) selfOrThrowIfLocked();
    }

    public T skipMemoryCache(boolean z2) {
        if (this.isAutoCloneEnabled) {
            return (T) mo7079clone().skipMemoryCache(true);
        }
        this.isCacheable = !z2;
        this.fields = (-1) - (((-1) - this.fields) & ((-1) - 256));
        return (T) selfOrThrowIfLocked();
    }

    public T theme(Resources.Theme theme) {
        if (this.isAutoCloneEnabled) {
            return (T) mo7079clone().theme(theme);
        }
        this.theme = theme;
        this.fields |= 32768;
        return (T) selfOrThrowIfLocked();
    ...
    T transform(Transformation<Bitmap> transformation, boolean z2) {
        if (this.isAutoCloneEnabled) {
            return (T) mo7079clone().transform(transformation, z2);
        }
    ...
    final T transform(DownsampleStrategy downsampleStrategy, Transformation<Bitmap> transformation) {
        if (this.isAutoCloneEnabled) {
            return (T) mo7079clone().transform(downsampleStrategy, transformation);
        }
        downsample(downsampleStrategy);
    ...
    <Y> T transform(Class<Y> cls, Transformation<Y> transformation, boolean z2) {
        if (this.isAutoCloneEnabled) {
            return (T) mo7079clone().transform(cls, transformation, z2);
        }
    ...
    public T useAnimationPool(boolean z2) {
        if (this.isAutoCloneEnabled) {
            return (T) mo7079clone().useAnimationPool(z2);
        }
    ...
    public T useUnlimitedSourceGeneratorsPool(boolean z2) {
        if (this.isAutoCloneEnabled) {
            return (T) mo7079clone().useUnlimitedSourceGeneratorsPool(z2);
        }
        this.useUnlimitedSourceGeneratorsPool = z2;
        int i2 = this.fields;
        this.fields = (i2 + 262144) - (i2 & 262144);
        return (T) selfOrThrowIfLocked();
    }
}
