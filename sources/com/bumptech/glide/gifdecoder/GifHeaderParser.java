...
public class GifHeaderParser {
    ...
    private void readBitmap() {
        ...
        int i2 = read();
        boolean z2 = (-1) - (((-1) - i2) | ((-1) - 128)) != 0;
        int iPow = (int) Math.pow(2.0d, (7 & i2) + 1);
        this.header.currentFrame.interlace = (-1) - (((-1) - i2) | ((-1) - 64)) != 0;
        if (z2) {
        ...
        while (true) {
            try {
                ...
                if (Log.isLoggable(TAG, 3)) {
                    String str = "Error Reading Block n: " + i3 + " count: " + i4 + " blockSize: " + this.blockSize;
                }
                this.header.status = 1;
                return;
            }
    ...
    private int[] readColorTable(int i2) {
        ...
        try {
            ...
            for (int i4 = 0; i4 < i2; i4++) {
                ...
                int i8 = bArr[i6] & 255;
                int i9 = i5 << 16;
                int i10 = (i9 + ViewCompat.MEASURED_STATE_MASK) - (i9 & ViewCompat.MEASURED_STATE_MASK);
                int i11 = i7 << 8;
                int i12 = (i11 + i10) - (i11 & i10);
                iArr[i4] = (i12 + i8) - (i12 & i8);
            }
    ...
    private void readGraphicControlExt() {
        read();
        int i2 = read();
        this.header.currentFrame.dispose = (28 & i2) >> 2;
        if (this.header.currentFrame.dispose == 0) {
            this.header.currentFrame.dispose = 1;
        }
        this.header.currentFrame.transparency = (-1) - (((-1) - i2) | ((-1) - 1)) != 0;
        int i3 = readShort();
        if (i3 < 2) {
            i3 = 10;
        }
        this.header.currentFrame.delay = i3 * 10;
    ...
    private void readLSD() {
        ...
        int i2 = read();
        this.header.gctFlag = (i2 + 128) - (128 | i2) != 0;
        this.header.gctSize = (int) Math.pow(2.0d, ((-1) - (((-1) - i2) | ((-1) - 7))) + 1);
    ...
    private void readNetscapeExt() {
        do {
            ...
            if (bArr[0] == 1) {
                int i2 = bArr[1] & 255;
                byte b2 = bArr[2];
                this.header.loopCount = (((b2 + 255) - (b2 | 255)) << 8) | i2;
            }
            if (this.blockSize <= 0) {
                return;
            }
