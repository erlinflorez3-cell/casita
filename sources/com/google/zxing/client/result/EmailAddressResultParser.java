package com.google.zxing.client.result;

import androidx.constraintlayout.core.motion.utils.TypedValues;
import androidx.core.net.MailTo;
import com.google.zxing.Result;
import java.util.Map;
import java.util.regex.Pattern;

/* JADX INFO: loaded from: classes7.dex */
public final class EmailAddressResultParser extends ResultParser {
    private static final Pattern COMMA = Pattern.compile(",");

    @Override // com.google.zxing.client.result.ResultParser
    public EmailAddressParsedResult parse(Result result) {
        String[] strArrSplit;
        String[] strArrSplit2;
        String str;
        String str2;
        String str3;
        String massagedText = getMassagedText(result);
        if (!massagedText.startsWith(MailTo.MAILTO_SCHEME) && !massagedText.startsWith("MAILTO:")) {
            if (EmailDoCoMoResultParser.isBasicallyValidEmailAddress(massagedText)) {
                return new EmailAddressParsedResult(massagedText);
            }
            return null;
        }
        String strSubstring = massagedText.substring(7);
        int iIndexOf = strSubstring.indexOf(63);
        if (iIndexOf >= 0) {
            strSubstring = strSubstring.substring(0, iIndexOf);
        }
        try {
            String strUrlDecode = urlDecode(strSubstring);
            String[] strArrSplit3 = !strUrlDecode.isEmpty() ? COMMA.split(strUrlDecode) : null;
            Map<String, String> nameValuePairs = parseNameValuePairs(massagedText);
            if (nameValuePairs != null) {
                if (strArrSplit3 == null && (str3 = nameValuePairs.get(TypedValues.TransitionType.S_TO)) != null) {
                    strArrSplit3 = COMMA.split(str3);
                }
                String str4 = nameValuePairs.get("cc");
                strArrSplit = str4 != null ? COMMA.split(str4) : null;
                String str5 = nameValuePairs.get("bcc");
                strArrSplit2 = str5 != null ? COMMA.split(str5) : null;
                str = nameValuePairs.get("subject");
                str2 = nameValuePairs.get("body");
            } else {
                strArrSplit = null;
                strArrSplit2 = null;
                str = null;
                str2 = null;
            }
            return new EmailAddressParsedResult(strArrSplit3, strArrSplit, strArrSplit2, str, str2);
        } catch (IllegalArgumentException unused) {
            return null;
        }
    }
}
