...
public final class ITFReader extends OneDReader {
    ...
    private static final int f3543w = 2;
    ...
    private static int decodeDigit(int[] iArr) throws NotFoundException {
        ...
        for (int i3 = 0; i3 < length; i3++) {
            float fPatternMatchVariance = patternMatchVariance(iArr, PATTERNS[i3], 0.5f);
    ...
    private void validateQuietZone(BitArray bitArray, int i2) throws NotFoundException {
        int iMin = Math.min(this.narrowLineWidth * 10, i2);
        for (int i3 = i2 - 1; iMin > 0 && i3 >= 0 && !bitArray.get(i3); i3--) {
            iMin--;
        }
        if (iMin != 0) {
            throw NotFoundException.getNotFoundInstance();
