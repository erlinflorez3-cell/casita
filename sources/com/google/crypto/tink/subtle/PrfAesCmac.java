...
public final class PrfAesCmac implements Prf {
    ...
    public byte[] compute(final byte[] data, int outputLength) throws GeneralSecurityException {
        ...
        byte[] bArrXor = iMax * 16 == data.length ? Bytes.xor(data, (iMax - 1) * 16, this.subKey1, 0, 16) : Bytes.xor(AesUtil.cmacPad(Arrays.copyOfRange(data, (iMax - 1) * 16, data.length)), this.subKey2);
        byte[] bArrDoFinal = new byte[16];
        for (int i2 = 0; i2 < iMax - 1; i2++) {
            bArrDoFinal = cipherInstance.doFinal(Bytes.xor(bArrDoFinal, 0, data, i2 * 16, 16));
        }
