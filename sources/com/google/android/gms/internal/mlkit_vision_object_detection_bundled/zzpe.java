package com.google.android.gms.internal.mlkit_vision_object_detection_bundled;

import com.google.firebase.encoders.annotations.Encodable;

/* JADX INFO: loaded from: classes8.dex */
@Encodable
public final class zzpe {
    private final zztk zza;
    private final zzpb zzb;
    private final zzoz zzc;
    private final zzro zzd;
    private final zzry zze;
    private final zzrv zzf;
    private final zzeo zzg;
    private final zzsy zzh;

    /* synthetic */ zzpe(zzpc zzpcVar, zzpd zzpdVar) {
        this.zza = zzpcVar.zza;
        this.zzb = zzpcVar.zzb;
        this.zzc = zzpcVar.zzc;
        this.zzd = zzpcVar.zzd;
        this.zze = zzpcVar.zze;
        this.zzf = zzpcVar.zzf;
        this.zzg = zzpcVar.zzg;
        this.zzh = zzpcVar.zzh;
    }

    public final zzeo zza() {
        return this.zzg;
    }

    public final zzoz zzb() {
        return this.zzc;
    }

    public final zzpb zzc() {
        return this.zzb;
    }

    public final zzro zzd() {
        return this.zzd;
    }

    public final zzrv zze() {
        return this.zzf;
    }

    public final zzry zzf() {
        return this.zze;
    }

    public final zzsy zzg() {
        return this.zzh;
    }

    public final zztk zzh() {
        return this.zza;
    }
}
