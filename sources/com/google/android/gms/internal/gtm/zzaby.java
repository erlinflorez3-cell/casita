...
public enum zzaby implements zzxx {
    ...
    ST_IP_ADDRESS(QuickTimeMetadataDirectory.TAG_LOCATION_BODY),
    ST_CHIME_REPRESENTATIVE_TARGET_ID(QuickTimeMetadataDirectory.TAG_LOCATION_NOTE),
    ST_HARDWARE_ID(1400),
    ST_ANDROID_LOGGING_ID(1401),
    ST_SOFTWARE_ID(ConnectionResult.DRIVE_EXTERNAL_STORAGE_REQUIRED),
    ST_USER_AGENT(1501),
    ST_ANONYMOUS_DATA(IDStepResponseCodes.STEP_LAUNCH_REQUEST),
    ST_DEMOGRAPHIC_INFO(1601),
    ST_LOCATION(IDStepResponseCodes.STEP_REQUEST),
    ...
    public static zzaby zzc(int i2) {
        if (i2 != 0) {
            if (i2 == 1211) {
                return ST_CHD_PAN;
            }
