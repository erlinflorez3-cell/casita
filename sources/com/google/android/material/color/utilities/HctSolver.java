...
public class HctSolver {
    ...
    static double[] bisectToLimit(double d2, double d3) {
        ...
        for (int i2 = 0; i2 < 3; i2++) {
            ...
            if (d4 != d5) {
                ...
                for (int i3 = 0; i3 < 8 && Math.abs(iCriticalPlaneBelow - iCriticalPlaneAbove) > 1; i3++) {
                    int iFloor = (int) Math.floor(((double) (iCriticalPlaneAbove + iCriticalPlaneBelow)) / 2.0d);
    ...
    static int findResultByJ(double d2, double d3, double d4) {
        ...
        for (int i2 = 0; i2 < 5; i2++) {
            ...
            double aw = (viewingConditions.getAw() * Math.pow(d5, (1.0d / viewingConditions.getC()) / viewingConditions.getZ())) / viewingConditions.getNbb();
            double d6 = (((0.305d + aw) * 23.0d) * dPow2) / (((23.0d * dCos) + ((dPow2 * 11.0d) * dCos2)) + ((dPow2 * 108.0d) * dSin));
            double d7 = d6 * dCos2;
            double d8 = d6 * dSin;
            double d9 = aw * 460.0d;
            double[] dArrMatrixMultiply = MathUtils.matrixMultiply(new double[]{inverseChromaticAdaptation((((451.0d * d7) + d9) + (288.0d * d8)) / 1403.0d), inverseChromaticAdaptation(((d9 - (891.0d * d7)) - (261.0d * d8)) / 1403.0d), inverseChromaticAdaptation(((d9 - (d7 * 220.0d)) - (d8 * 6300.0d)) / 1403.0d)}, LINRGB_FROM_SCALED_DISCOUNT);
            ...
            double d13 = (dArr[0] * d10) + (dArr[1] * d11) + (dArr[2] * d12);
            if (d13 <= 0.0d) {
                return 0;
            }
            if (i2 != 4) {
