...
public final class Score {
    ...
    public static List<Integer> score(Map<Integer, Integer> map, int i2, int i3, boolean z2) {
        ...
        int[] iArr = new int[SessionSplitConfiguration.DEFAULT_MAX_SESSION_DURATION_MINS];
        double d2 = 0.0d;
        ...
        double[] dArr = new double[SessionSplitConfiguration.DEFAULT_MAX_SESSION_DURATION_MINS];
        ...
        for (Hct hct : arrayList) {
            ...
            if (!z2 || (hct.getChroma() >= CUTOFF_CHROMA && d4 > CUTOFF_EXCITED_PROPORTION)) {
                arrayList2.add(new ScoredHCT(hct, (d4 * 100.0d * 0.7d) + ((hct.getChroma() - TARGET_CHROMA) * (hct.getChroma() < TARGET_CHROMA ? 0.1d : WEIGHT_CHROMA_ABOVE))));
            }
        }
