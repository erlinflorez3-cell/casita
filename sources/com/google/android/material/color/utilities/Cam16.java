...
public final class Cam16 {
    ...
    private final double f3306j;
    private final double jstar;

    /* JADX INFO: renamed from: m */
    private final double f3307m;

    /* JADX INFO: renamed from: q */
    private final double f3308q;

    /* JADX INFO: renamed from: s */
    private final double f3309s;
    ...
    static Cam16 fromIntInViewingConditions(int i2, ViewingConditions viewingConditions) {
        double dLinearized = ColorUtils.linearized(((-1) - (((-1) - 16711680) | ((-1) - i2))) >> 16);
        double dLinearized2 = ColorUtils.linearized(((65280 + i2) - (65280 | i2)) >> 8);
    ...
    static Cam16 fromXyzInViewingConditions(double d2, double d3, double d4, ViewingConditions viewingConditions) {
        ...
        double c2 = (4.0d / viewingConditions.getC()) * Math.sqrt(d16) * (viewingConditions.getAw() + 4.0d) * viewingConditions.getFlRoot();
        double dPow5 = Math.pow(1.64d - Math.pow(0.29d, viewingConditions.getN()), 0.73d) * Math.pow(((((((Math.cos(Math.toRadians(degrees < 20.14d ? degrees + 360.0d : degrees) + 2.0d) + 3.8d) * 0.25d) * 3846.153846153846d) * viewingConditions.getNc()) * viewingConditions.getNcb()) * Math.hypot(d11, d12)) / (d14 + 0.305d), 0.9d);
        ...
        return new Cam16(degrees, dSqrt, dPow4, c2, flRoot, Math.sqrt((dPow5 * viewingConditions.getC()) / (viewingConditions.getAw() + 4.0d)) * 50.0d, (1.7000000000000002d * dPow4) / ((0.007d * dPow4) + 1.0d), dLog1p * Math.cos(radians), dLog1p * Math.sin(radians));
    }
