...
class KeylineStateList {
    ...
    private static List<KeylineState> getStateStepsStart(Carousel carousel, KeylineState keylineState, float f2, float f3) {
        ...
        while (i3 < firstFocalKeylineIndex) {
            ...
            f5 += keylineState.getKeylines().get(i4).cutoff;
            int i5 = i4 - i2;
            if (i5 >= 0) {
                size = findFirstIndexAfterLastFocalKeylineWithMask(keylineState2, keylineState.getKeylines().get(i5).mask) - i2;
            }
            KeylineState keylineStateMoveKeylineAndCreateKeylineState = moveKeylineAndCreateKeylineState(keylineState2, iFindFirstNonAnchorKeylineIndex, size, f4 + f5, (keylineState.getFirstFocalKeylineIndex() - i3) - 1, (keylineState.getLastFocalKeylineIndex() - i3) - 1, containerWidth);
            if (i3 == firstFocalKeylineIndex - 1 && f3 > 0.0f) {
    ...
    private static KeylineState moveKeylineAndCreateKeylineState(KeylineState keylineState, int i2, int i3, float f2, int i4, int i5, float f3) {
        ...
        while (i6 < arrayList.size()) {
            ...
            builder.addKeyline(f2 + (keyline.maskedItemSize / 2.0f), keyline.mask, keyline.maskedItemSize, i6 >= i4 && i6 <= i5, keyline.isAnchor, keyline.cutoff);
    ...
    private static KeylineState shiftKeylineStateForPadding(KeylineState keylineState, float f2, float f3, boolean z2, float f4) {
        ...
        while (i2 < arrayList.size()) {
            ...
            if (keyline.isAnchor) {
                ...
                float childMaskPercentage = CarouselStrategy.getChildMaskPercentage(f5, keylineState.getItemSize(), f4);
                float f6 = (f5 / 2.0f) + f2;
                float f7 = f6 - keyline.locOffset;
                float f8 = keyline.cutoff;
                float f9 = z2 ? f7 : 0.0f;
                if (z2) {
                    f7 = 0.0f;
                }
                builder.addKeyline(f6, childMaskPercentage, f5, z3, false, f8, f9, f7);
                f2 += f5;
            }
            i2++;
        }
    ...
    Map<Integer, KeylineState> getKeylineStateForPositionMap(int i2, int i3, int i4, boolean z2) {
    ...
    KeylineState getShiftedState(float f2, float f3, float f4, boolean z2) {
        float fLerp;
