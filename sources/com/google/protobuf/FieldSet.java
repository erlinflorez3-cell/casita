...
final class FieldSet<T extends FieldDescriptorLite<T>> {
        ...
        private void mergeFromField(final Map.Entry<T, Object> entry) {
            ...
            if (key.isRepeated()) {
                ...
                for (int i2 = 0; i2 < size; i2++) {
                    arrayList.add(FieldSet.cloneIfMutable(list.get(i2)));
                }
                return;
            }
    ...
    public static <T extends FieldDescriptorLite<T>> SmallSortedMap<T, Object> cloneAllFieldsMap(SmallSortedMap<T, Object> fields, boolean copyList, boolean resolveLazyFields) {
        ...
        for (int i2 = 0; i2 < numArrayEntries; i2++) {
            cloneFieldEntry(smallSortedMapNewFieldMap, fields.getArrayEntryAt(i2), copyList, resolveLazyFields);
        }
    ...
    public static int computeFieldSize(final FieldDescriptorLite<?> descriptor, final Object value) {
        ...
        if (!descriptor.isPacked()) {
            ...
            while (i2 < size) {
                iComputeElementSize += computeElementSize(liteType, number, list.get(i2));
                i2++;
            }
    ...
    public static <T extends FieldDescriptorLite<T>> boolean isInitialized(final Map.Entry<T, Object> entry) {
        ...
        int size = list.size();
        for (int i2 = 0; i2 < size; i2++) {
            if (!isMessageFieldValueInitialized(list.get(i2))) {
                return false;
            }
        }
        return true;
    ...
    private void mergeFromField(final Map.Entry<T, Object> entry) {
        ...
        if (key.isRepeated()) {
            ...
            for (int i2 = 0; i2 < size; i2++) {
                list2.add(cloneIfMutable(list.get(i2)));
            }
    ...
    public static void writeField(final FieldDescriptorLite<?> descriptor, final Object value, final CodedOutputStream output) throws IOException {
        ...
        if (!descriptor.isPacked()) {
            while (i2 < size) {
                writeElement(output, liteType, number, list.get(i2));
                i2++;
            }
        } else {
            if (list.isEmpty()) {
            ...
            for (int i3 = 0; i3 < size; i3++) {
                iComputeElementSizeNoTag += computeElementSizeNoTag(liteType, list.get(i3));
            }
            output.writeUInt32NoTag(iComputeElementSizeNoTag);
            while (i2 < size) {
                writeElementNoTag(output, liteType, list.get(i2));
                i2++;
            }
        }
    }
    ...
    public void makeImmutable() {
        ...
        int numArrayEntries = this.fields.getNumArrayEntries();
        for (int i2 = 0; i2 < numArrayEntries; i2++) {
            Object value = this.fields.getArrayEntryAt(i2).getValue();
