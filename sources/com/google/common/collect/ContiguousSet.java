...
public abstract class ContiguousSet<C extends Comparable> extends ImmutableSortedSet<C> {
    ...
    public static <C extends Comparable> ContiguousSet<C> create(Range<C> range, DiscreteDomain<C> domain) {
        ...
        try {
            ...
            return (!rangeIntersection.isEmpty() && Range.compareOrThrow((Comparable) Objects.requireNonNull(range.lowerBound.leastValueAbove(domain)), (Comparable) Objects.requireNonNull(range.upperBound.greatestValueBelow(domain))) <= 0) ? new RegularContiguousSet(rangeIntersection, domain) : new EmptyContiguousSet(domain);
