...
public final class Queues {
    ...
    public static <E> int drain(BlockingQueue<E> q2, Collection<? super E> buffer, int numElements, long timeout, TimeUnit unit) throws InterruptedException {
        ...
        while (iDrainTo < numElements) {
            iDrainTo += q2.drainTo(buffer, numElements - iDrainTo);
            if (iDrainTo < numElements) {
                E ePoll = q2.poll(jNanoTime - System.nanoTime(), TimeUnit.NANOSECONDS);
                if (ePoll == null) {
                    break;
                }
                buffer.add(ePoll);
        ...
        while (iDrainTo < numElements) {
            try {
                ...
                if (iDrainTo < numElements) {
                    while (true) {
                        try {
                            ePoll = q2.poll(jNanoTime - System.nanoTime(), TimeUnit.NANOSECONDS);
                            break;
