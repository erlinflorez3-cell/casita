...
public final class LinkedHashMultimap<K, V> extends LinkedHashMultimapGwtSerializationDependencies<K, V> {
    ...
    static final class ValueEntry<K, V> extends ImmutableEntry<K, V> implements ValueSetLink<K, V> {
        ...
        boolean matchesValue(@CheckForNull Object v2, int smearedVHash) {
            return this.smearedValueHash == smearedVHash && com.google.common.base.Objects.equal(getValue(), v2);
        }
    ...
    final class ValueSet extends Sets.ImprovedAbstractSet<V> implements ValueSetLink<K, V> {
        ...
        private void rehashIfNecessary() {
            if (Hashing.needsResizing(this.size, this.hashTable.length, 1.0d)) {
                ...
                for (ValueSetLink<K, V> successorInValueSet = this.firstEntry; successorInValueSet != this; successorInValueSet = successorInValueSet.getSuccessorInValueSet()) {
                    ...
                    int i3 = (-1) - (((-1) - valueEntry.smearedValueHash) | ((-1) - i2));
        ...
        public boolean contains(@CheckForNull Object o2) {
            ...
            for (ValueEntry<K, V> valueEntry = this.hashTable[(-1) - (((-1) - mask()) | ((-1) - iSmearedHash))]; valueEntry != null; valueEntry = valueEntry.nextInValueBucket) {
                if (valueEntry.matchesValue(o2, iSmearedHash)) {
                    return true;
                }
            }
            return false;
        ...
        public boolean remove(@CheckForNull Object o2) {
            ...
            for (ValueEntry<K, V> valueEntry2 = this.hashTable[iMask]; valueEntry2 != null; valueEntry2 = valueEntry2.nextInValueBucket) {
                if (valueEntry2.matchesValue(o2, iSmearedHash)) {
                    if (valueEntry == null) {
    ...
    private void readObject(ObjectInputStream stream) throws ClassNotFoundException, IOException {
        ...
        for (int i5 = 0; i5 < i4; i5++) {
            ((Collection) Objects.requireNonNull((Collection) mapNewLinkedHashMapWithExpectedSize.get(stream.readObject()))).add(stream.readObject());
        }
