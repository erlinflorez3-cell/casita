...
public final class Graphs {
            ...
            public Iterator<EndpointPair<N>> iterator() {
                return Iterators.transform(TransposedGraph.this.delegate().incidentEdges(this.node).iterator(), new Function() { // from class: com.google.common.graph.Graphs$TransposedGraph$1$$ExternalSyntheticLambda0
                    ...
                    public final Object apply(Object obj) {
                        return this.f$0.m7681x5749e417((EndpointPair) obj);
                    }
                });
            }

            /* JADX INFO: renamed from: lambda$iterator$0$com-google-common-graph-Graphs$TransposedGraph$1 */
            /* synthetic */ EndpointPair m7681x5749e417(EndpointPair endpointPair) {
    ...
    public static <N> MutableGraph<N> inducedSubgraph(Graph<N> graph, Iterable<? extends N> iterable) {
        ...
        for (N n2 : standardMutableGraph.nodes()) {
            for (N n3 : graph.successors((Object) n2)) {
    ...
    public static <N, V> MutableValueGraph<N, V> inducedSubgraph(ValueGraph<N, V> valueGraph, Iterable<? extends N> iterable) {
        ...
        for (N n2 : mutableValueGraph.nodes()) {
            for (N n3 : valueGraph.successors((Object) n2)) {
    ...
    private static <N> boolean subgraphHasCycle(Graph<N> graph, Map<Object, NodeVisitState> visitedNodes, N node, @CheckForNull N previousNode) {
        ...
        for (N n2 : graph.successors((Object) node)) {
            if (canTraverseWithoutReusingEdge(graph, n2, previousNode) && subgraphHasCycle(graph, visitedNodes, n2, node)) {
                return true;
            }
        }
    ...
    public static <N> Graph<N> transitiveClosure(Graph<N> graph) {
        ...
        if (graph.isDirected()) {
            for (N n2 : graph.nodes()) {
                ...
                while (it.hasNext()) {
                    standardMutableGraphBuild.putEdge(n2, it.next());
                }
            }
        } else {
    ...
    public static <N, V> ValueGraph<N, V> transpose(ValueGraph<N, V> graph) {
        return !graph.isDirected() ? graph : graph instanceof TransposedValueGraph ? ((TransposedValueGraph) graph).graph : new TransposedValueGraph(graph);
    }
}
