...
final class StandardMutableNetwork<N, E> extends StandardNetwork<N, E> implements MutableNetwork<N, E> {
    ...
    public boolean removeEdge(E edge) {
        Preconditions.checkNotNull(edge, "edge");
        N n2 = this.edgeToReferenceNode.get(edge);
        boolean z2 = false;
        if (n2 == null) {
            return false;
        }
        NetworkConnections networkConnections = (NetworkConnections) Objects.requireNonNull(this.nodeConnections.get(n2));
        ...
        if (allowsSelfLoops() && n2.equals(objAdjacentNode)) {
            z2 = true;
        }
        networkConnections2.removeInEdge(edge, z2);
