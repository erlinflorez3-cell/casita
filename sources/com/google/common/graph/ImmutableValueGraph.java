...
public final class ImmutableValueGraph<N, V> extends StandardValueGraph<N, V> {
    ...
    private static <N, V> GraphConnections<N, V> connectionsOf(final ValueGraph<N, V> graph, final N node) {
        ...
        return graph.isDirected() ? DirectedGraphConnections.ofImmutable(node, graph.incidentEdges(node), function) : UndirectedGraphConnections.ofImmutable(Maps.asMap(graph.adjacentNodes(node), function));
    }
