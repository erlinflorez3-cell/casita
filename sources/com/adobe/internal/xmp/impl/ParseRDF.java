...
public class ParseRDF implements XMPError, XMPConst {
    ...
    private static boolean isNumberedArrayItemName(String str) {
        ...
        if (str.startsWith("rdf:_")) {
            ...
            for (int i2 = 5; i2 < str.length(); i2++) {
                zEquals = zEquals && str.charAt(i2) >= '0' && str.charAt(i2) <= '9';
            }
        }
        return zEquals;
    }
    ...
    */
    private static void rdf_EmptyPropertyElement(com.adobe.internal.xmp.impl.XMPMetaImpl r18, com.adobe.internal.xmp.impl.XMPNode r19, org.w3c.dom.Node r20, boolean r21) throws com.adobe.internal.xmp.XMPException {
        /*
    ...
    private static void rdf_LiteralPropertyElement(XMPMetaImpl xMPMetaImpl, XMPNode xMPNode, Node node, boolean z2) throws XMPException {
        XMPNode xMPNodeAddChildNode = addChildNode(xMPMetaImpl, xMPNode, node, null, z2);
        ...
        for (int i3 = 0; i3 < node.getChildNodes().getLength(); i3++) {
            Node nodeItem2 = node.getChildNodes().item(i3);
    ...
    private static void rdf_NodeElementAttrs(XMPMetaImpl xMPMetaImpl, XMPNode xMPNode, Node node, boolean z2, ParseOptions parseOptions) throws XMPException {
        ...
        for (int i3 = 0; i3 < node.getAttributes().getLength(); i3++) {
            Node nodeItem = node.getAttributes().item(i3);
            if (!"xmlns".equals(nodeItem.getPrefix()) && (nodeItem.getPrefix() != null || !"xmlns".equals(nodeItem.getNodeName()))) {
                int rDFTermKind = getRDFTermKind(nodeItem);
                if (rDFTermKind == 0) {
                    addChildNode(xMPMetaImpl, xMPNode, nodeItem, nodeItem.getNodeValue(), z2);
                } else {
                    ...
                    if (z2 && rDFTermKind == 3) {
                        ...
                        } else if (!xMPNode.getName().equals(nodeItem.getNodeValue())) {
    ...
    private static void rdf_ParseTypeResourcePropertyElement(XMPMetaImpl xMPMetaImpl, XMPNode xMPNode, Node node, boolean z2, ParseOptions parseOptions) throws XMPException {
        XMPNode xMPNodeAddChildNode = addChildNode(xMPMetaImpl, xMPNode, node, "", z2);
        ...
        for (int i2 = 0; i2 < node.getChildNodes().getLength(); i2++) {
            ...
            if (!isWhitespaceNode(nodeItem)) {
                ...
                if (xMPNode.getOptions().isArrayLimited() && i2 > xMPNode.getOptions().getArrayElementsLimit()) {
                    ...
                    rdf_PropertyElement(xMPMetaImpl, xMPNode, nodeItem, z2, parseOptions);
                }
            }
        }
    }
