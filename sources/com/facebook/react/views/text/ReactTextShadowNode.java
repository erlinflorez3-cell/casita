...
public class ReactTextShadowNode extends ReactBaseTextShadowNode {
    ...
    public ReactTextShadowNode(ReactTextViewManagerCallback reactTextViewManagerCallback) {
        ...
        this.mTextMeasureFunction = new YogaMeasureFunction() { // from class: com.facebook.react.views.text.ReactTextShadowNode.1
            ...
            */
            public long measure(com.facebook.yoga.YogaNode r18, float r19, com.facebook.yoga.YogaMeasureMode r20, float r21, com.facebook.yoga.YogaMeasureMode r22) throws java.lang.Throwable {
                /*
        ...
        this.mTextBaselineFunction = new YogaBaselineFunction() { // from class: com.facebook.react.views.text.ReactTextShadowNode.2
            ...
            public float baseline(YogaNode yogaNode, float f2, float f3) {
                Layout layoutMeasureSpannedText = ReactTextShadowNode.this.measureSpannedText((Spannable) Assertions.assertNotNull(ReactTextShadowNode.this.mPreparedSpannableText, "Spannable element has not been prepared in onBeforeLayout"), f2, YogaMeasureMode.EXACTLY);
    ...
    public Layout measureSpannedText(Spannable spannable, float f2, YogaMeasureMode yogaMeasureMode) {
        ...
        boolean z2 = yogaMeasureMode == YogaMeasureMode.UNDEFINED || fCeil < 0.0f;
        ...
        if (metricsIsBoring == null && (z2 || (!YogaConstants.isUndefined(desiredWidth) && desiredWidth <= fCeil))) {
            StaticLayout.Builder hyphenationFrequency = StaticLayout.Builder.obtain(spannable, 0, spannable.length(), textPaint, (int) Math.ceil(desiredWidth)).setAlignment(alignment).setLineSpacing(0.0f, 1.0f).setIncludePad(this.mIncludeFontPadding).setBreakStrategy(this.mTextBreakStrategy).setHyphenationFrequency(this.mHyphenationFrequency);
        ...
        if (metricsIsBoring != null && (z2 || metricsIsBoring.width <= fCeil)) {
            return BoringLayout.make(spannable, textPaint, Math.max(metricsIsBoring.width, 0), alignment, 1.0f, 0.0f, metricsIsBoring, this.mIncludeFontPadding);
        }
        if (Build.VERSION.SDK_INT > 29) {
            fCeil = (float) Math.ceil(fCeil);
        }
        StaticLayout.Builder hyphenationFrequency2 = StaticLayout.Builder.obtain(spannable, 0, spannable.length(), textPaint, (int) fCeil).setAlignment(alignment).setLineSpacing(0.0f, 1.0f).setIncludePad(this.mIncludeFontPadding).setBreakStrategy(this.mTextBreakStrategy).setHyphenationFrequency(this.mHyphenationFrequency);
        if (Build.VERSION.SDK_INT >= 28) {
    ...
    @InterfaceC1492Gx
    @ReactProp(customType = ">pgAIg_>{\u0006\u0019Dh_e\u000b", name = "\u0010]S~\u0014<yyoEE*")
