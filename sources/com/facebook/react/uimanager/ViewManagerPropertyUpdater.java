...
public class ViewManagerPropertyUpdater {
    ...
    private static class FallbackShadowNodeSetter<T extends ReactShadowNode> implements ShadowNodeSetter<T> {
        ...
        /* synthetic */ FallbackShadowNodeSetter(Class cls, FallbackShadowNodeSetterIA fallbackShadowNodeSetterIA) {
            this(cls);
        }
    ...
    private static class FallbackViewManagerSetter<T extends ViewManager, V extends View> implements ViewManagerSetter<T, V> {
        ...
        /* synthetic */ FallbackViewManagerSetter(Class cls, FallbackViewManagerSetterIA fallbackViewManagerSetterIA) {
            this(cls);
        }
    ...
    public static <T extends ReactShadowNode> void updateProps(T t2, ReactStylesDiffMap reactStylesDiffMap) {
        ShadowNodeSetter shadowNodeSetterFindNodeSetter = findNodeSetter(t2.getClass());
        Iterator<Map.Entry<String, Object>> entryIterator = reactStylesDiffMap.mBackingMap.getEntryIterator();
        while (entryIterator.hasNext()) {
            Map.Entry<String, Object> next = entryIterator.next();
            shadowNodeSetterFindNodeSetter.setProperty(t2, next.getKey(), next.getValue());
        }
    }

    public static <T extends ViewManager, V extends View> void updateProps(T t2, V v2, ReactStylesDiffMap reactStylesDiffMap) {
        ViewManagerSetter viewManagerSetterFindManagerSetter = findManagerSetter(t2.getClass());
        Iterator<Map.Entry<String, Object>> entryIterator = reactStylesDiffMap.mBackingMap.getEntryIterator();
        while (entryIterator.hasNext()) {
            Map.Entry<String, Object> next = entryIterator.next();
            viewManagerSetterFindManagerSetter.setProperty(t2, v2, next.getKey(), next.getValue());
        }
    }
