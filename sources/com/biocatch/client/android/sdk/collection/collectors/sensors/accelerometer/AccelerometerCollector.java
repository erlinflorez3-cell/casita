...
public final class AccelerometerCollector extends InteractionRelativeSensorEvent<AccelerometerEventModel> {
    ...
    public AccelerometerCollector(SensorService sensorService, Handler sensorThreadHandler, SensorGateKeeper sensorGateKeeper, DataQueue historyBuffer, Utils utils, EventsSubscriber eventsSubscriber, AccelerometerCollectorSettings accelerometerSettings) {
        super(historyBuffer, sensorService, sensorGateKeeper, sensorThreadHandler, utils, eventsSubscriber, accelerometerSettings);
        ...
        Intrinsics.checkNotNullParameter(accelerometerSettings, "accelerometerSettings");
        this.accelerometerSettings = accelerometerSettings;
        this.threshold = accelerometerSettings.getThreshold();
    }
    ...
    public synchronized void configure(ConfigurationRepository configurationRepository) {
        ...
        this.threshold = this.accelerometerSettings.getThreshold();
    }
    ...
    public ConfigKeys getConfigKey() {
        return ConfigKeys.isAccelerometerEvents;
    }
    ...
    public void handleSensorChange(SensorEvent event) throws JSONException {
        ...
        this.lastY = d3;
        this.lastZ = d4;
        double dTruncate = getUtils().truncate(d2, getSensorPrecision());
        double dTruncate2 = getUtils().truncate(d3, getSensorPrecision());
        double dTruncate3 = getUtils().truncate(d4, getSensorPrecision());
        if (dSqrt >= this.threshold) {
            addToBuffer(new AccelerometerEventModel(getContextIDCache().get(), getCurrentEventId(), getUtils().upTimeToEpoch(event.timestamp), dTruncate, dTruncate2, dTruncate3));
        }
    }
    ...
    public void start() {
        if (isStarted()) {
            Log.Companion.getLogger().error("Accelerometer events collector is already started. Aborting the start operation.");
            throw new InvalidOperationException("Accelerometer events collector is already started. Aborting the start operation.");
        }
        super.start();
        setStarted(true);
    }
