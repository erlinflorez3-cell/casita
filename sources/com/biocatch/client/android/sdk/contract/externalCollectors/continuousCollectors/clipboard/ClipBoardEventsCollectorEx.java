...
import com.biocatch.client.android.sdk.collection.collectors.clipboard.ClipboardEventModel;
...
public abstract class ClipBoardEventsCollectorEx extends ContinuousCollectorEx<ClipboardEventModelEx> {
    ...
    public void onCollectedData(ClipboardEventModelEx collectionItem) {
        ...
        try {
            ...
            int i2 = getContextIDCache().get();
            long jCurrentTimeMillis = System.currentTimeMillis();
            ClipEventType eventType = collectionItem.getEventType();
            int hash = iFocusManagerProvideFocusManager.getCurrentFocus().getHash();
            String id = iFocusManagerProvideFocusManager.getCurrentFocus().getId();
            String text = collectionItem.getText();
            if (text == null) {
                text = "";
            }
            addToQueue(new ClipboardEventModel(i2, currentEventId, jCurrentTimeMillis, eventType, hash, textMaskingServiceProvideTextMaskingService.maskClipBoardText(id, text)));
        } catch (Throwable th) {
            Log.Companion.getLogger().error("Clipboard masking failed, disregarding the clipboard event", th);
        }
    }
