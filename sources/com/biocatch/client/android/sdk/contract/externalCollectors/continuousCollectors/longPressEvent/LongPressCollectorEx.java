...
import com.biocatch.client.android.sdk.collection.collectors.gestures.LongPressEventModel;
...
public abstract class LongPressCollectorEx extends ContinuousCollectorEx<LongPressEventModelEx> {
    ...
    public ConfigKeys getConfigKey() {
        return ConfigKeys.isLongPressEvents;
    }
    ...
    public void onCollectedData(LongPressEventModelEx collectionItem) {
        Intrinsics.checkNotNullParameter(collectionItem, "collectionItem");
        addToQueue(new LongPressEventModel(getContextIDCache().get(), getCurrentEventId(), System.currentTimeMillis(), collectionItem.getX(), collectionItem.getY()));
    }

    @Override // com.biocatch.android.commonsdk.collection.collectors.ContinuousCollector
    public void start() {
        if (isStarted()) {
            Log.Companion.getLogger().error("Long Press events collector is already started. Aborting the start operation.");
            throw new InvalidOperationException("Long Press events collector is already started. Aborting the start operation.");
        }
        onStart();
        setStarted(true);
    }
