...
public abstract class ElementEventsCollectorEx extends ContinuousCollectorEx<ElementEventsModelEx> {
    ...
    public void onCollectedData(ElementEventsModelEx collectionItem) {
        Intrinsics.checkNotNullParameter(collectionItem, "collectionItem");
        try {
            IFocusManager iFocusManagerProvideFocusManager = SdkInjectorKt.getCommonInjector().provideFocusManager();
            TextMaskingService textMaskingServiceProvideTextMaskingService = SdkInjectorKt.getCommonInjector().provideTextMaskingService();
            if (collectionItem.getType() == ElementEventType.FOCUS) {
                iFocusManagerProvideFocusManager.setCurrentFocus(new FocusElement(collectionItem.getHash(), collectionItem.getElementID()));
            }
            addToQueue(new ElementEventModel(Integer.valueOf(getContextIDCache().get()), Long.valueOf(getCurrentEventId()), System.currentTimeMillis(), null, collectionItem.getType(), collectionItem.getHash(), null, Integer.valueOf(collectionItem.getElementValue().length()), textMaskingServiceProvideTextMaskingService.maskInputs(collectionItem.getElementID(), collectionItem.getElementValue()), 72, null));
        } catch (Throwable th) {
            Log.Companion.getLogger().error("Element event masking failed, disregarding the element event", th);
        }
    }
