...
public class PngMetadataReader {
    private static Set<PngChunkType> _desiredChunkTypes = null;
    ...
    private static void processChunk(Metadata metadata, PngChunk pngChunk) throws Throwable {
        ...
        if (type.equals(PngChunkType.IHDR)) {
            ...
            pngDirectory.setInt(5, (-1) - (((-1) - pngHeader.getCompressionType()) | ((-1) - 255)));
        ...
        if (type.equals(PngChunkType.gAMA)) {
            ...
            pngDirectory5.setDouble(11, ((double) int32BigEndian) / 100000.0d);
        ...
        if (type.equals(PngChunkType.iCCP)) {
            ...
            if (sequentialByteArrayReader.getInt8() == 0) {
                try {
                    InflaterInputStream inflaterInputStream = new InflaterInputStream(new ByteArrayInputStream(sequentialByteArrayReader.getBytes(bytes.length - (nullTerminatedBytes.length + 2))));
                    new IccReader().extract(new RandomAccessStreamReader(inflaterInputStream), metadata, pngDirectory6);
                    inflaterInputStream.close();
                } catch (ZipException e2) {
                    pngDirectory6.addError(String.format("Exception decompressing PNG iCCP chunk : %s", e2.getMessage()));
        ...
        if (type.equals(PngChunkType.tEXt)) {
            ...
            String string = nullTerminatedStringValue.toString();
            ...
            arrayList.add(new KeyValuePair(string, nullTerminatedStringValue2));
        ...
        if (type.equals(PngChunkType.zTXt)) {
            ...
            String string2 = nullTerminatedStringValue3.toString();
            byte int8 = sequentialByteArrayReader3.getInt8();
            int length = bytes.length - (nullTerminatedStringValue3.getBytes().length + 2);
            if (int8 == 0) {
                try {
                    allBytes = StreamUtil.readAllBytes(new InflaterInputStream(new ByteArrayInputStream(bytes, bytes.length - length, length)));
                } catch (ZipException e3) {
                    PngDirectory pngDirectory9 = new PngDirectory(PngChunkType.zTXt);
                    pngDirectory9.addError(String.format("Exception decompressing PNG zTXt chunk with keyword \"%s\": %s", string2, e3.getMessage()));
                ...
                metadata.addDirectory(pngDirectory10);
            }
            if (allBytes != null) {
                if (string2.equals("XML:com.adobe.xmp")) {
                    new XmpReader().extract(allBytes, metadata);
                    return;
                }
                ArrayList arrayList2 = new ArrayList();
                arrayList2.add(new KeyValuePair(string2, new StringValue(allBytes, _latin1Encoding)));
                PngDirectory pngDirectory11 = new PngDirectory(PngChunkType.zTXt);
                pngDirectory11.setObject(13, arrayList2);
                metadata.addDirectory(pngDirectory11);
                return;
            }
            return;
        }
        if (type.equals(PngChunkType.iTXt)) {
            ...
            String string3 = nullTerminatedStringValue4.toString();
            byte int82 = sequentialByteArrayReader4.getInt8();
            byte int83 = sequentialByteArrayReader4.getInt8();
            int length2 = bytes.length - (((((nullTerminatedStringValue4.getBytes().length + 3) + sequentialByteArrayReader4.getNullTerminatedBytes(bytes.length).length) + 1) + sequentialByteArrayReader4.getNullTerminatedBytes(bytes.length).length) + 1);
            if (int82 == 0) {
                allBytes = sequentialByteArrayReader4.getNullTerminatedBytes(length2);
            } else if (int82 != 1) {
                PngDirectory pngDirectory12 = new PngDirectory(PngChunkType.iTXt);
                pngDirectory12.addError("Invalid compression flag value");
                metadata.addDirectory(pngDirectory12);
            } else if (int83 == 0) {
                try {
                    allBytes = StreamUtil.readAllBytes(new InflaterInputStream(new ByteArrayInputStream(bytes, bytes.length - length2, length2)));
                } catch (ZipException e4) {
                    PngDirectory pngDirectory13 = new PngDirectory(PngChunkType.iTXt);
                    pngDirectory13.addError(String.format("Exception decompressing PNG iTXt chunk with keyword \"%s\": %s", string3, e4.getMessage()));
                    metadata.addDirectory(pngDirectory13);
                }
            } else {
                PngDirectory pngDirectory14 = new PngDirectory(PngChunkType.iTXt);
                pngDirectory14.addError("Invalid compression method value");
                metadata.addDirectory(pngDirectory14);
            }
            if (allBytes != null) {
                if (string3.equals("XML:com.adobe.xmp")) {
                    new XmpReader().extract(allBytes, metadata);
                    return;
                }
                ArrayList arrayList3 = new ArrayList();
                arrayList3.add(new KeyValuePair(string3, new StringValue(allBytes, _latin1Encoding)));
                PngDirectory pngDirectory15 = new PngDirectory(PngChunkType.iTXt);
                pngDirectory15.setObject(13, arrayList3);
                metadata.addDirectory(pngDirectory15);
                return;
            }
            return;
        }
        if (type.equals(PngChunkType.tIME)) {
            ...
            if (DateUtil.isValidDate(uInt16, uInt8 - 1, uInt82) && DateUtil.isValidTime(uInt83, uInt84, uInt85)) {
                pngDirectory16.setString(14, String.format("%04d:%02d:%02d %02d:%02d:%02d", Integer.valueOf(uInt16), Integer.valueOf(uInt8), Integer.valueOf(uInt82), Integer.valueOf(uInt83), Integer.valueOf(uInt84), Integer.valueOf(uInt85)));
            } else {
                pngDirectory16.addError(String.format("PNG tIME data describes an invalid date/time: year=%d month=%d day=%d hour=%d minute=%d second=%d", Integer.valueOf(uInt16), Integer.valueOf(uInt8), Integer.valueOf(uInt82), Integer.valueOf(uInt83), Integer.valueOf(uInt84), Integer.valueOf(uInt85)));
            }
            metadata.addDirectory(pngDirectory16);
            return;
        }
        if (type.equals(PngChunkType.pHYs)) {
            ...
            metadata.addDirectory(pngDirectory17);
            return;
        }
        if (type.equals(PngChunkType.sBIT)) {
            ...
            metadata.addDirectory(pngDirectory18);
        } else if (type.equals(PngChunkType.eXIf)) {
            try {
                new TiffReader().processTiff(new ByteArrayReader(bytes), new ExifTiffHandler(metadata, null), 0);
            } catch (TiffProcessingException e5) {
                PngDirectory pngDirectory19 = new PngDirectory(PngChunkType.eXIf);
                pngDirectory19.addError(e5.getMessage());
                metadata.addDirectory(pngDirectory19);
            } catch (IOException e6) {
                PngDirectory pngDirectory20 = new PngDirectory(PngChunkType.eXIf);
                pngDirectory20.addError(e6.getMessage());
                metadata.addDirectory(pngDirectory20);
            }
        }
    }

    public static Metadata readMetadata(File file) throws IOException, PngProcessingException {
    ...
    public static Metadata readMetadata(InputStream inputStream) throws Throwable {
        ...
        Iterator<PngChunk> it = iterableExtract.iterator();
        while (it.hasNext()) {
            try {
                processChunk(metadata, it.next());
            } catch (Exception e2) {
                metadata.addDirectory(new ErrorDirectory("Exception reading PNG chunk: " + e2.getMessage()));
            }
        }
        return metadata;
    }
