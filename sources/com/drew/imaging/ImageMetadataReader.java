...
public class ImageMetadataReader {
    ...
    static /* synthetic */ class AnonymousClass1 {
        ...
        static {
            ...
            try {
                $SwitchMap$com$drew$imaging$FileType[FileType.QuickTime.ordinal()] = 18;
    ...
    public static void main(String[] strArr) {
        ...
        if (arrayList.size() < 1) {
            ...
            printStream.println(String.format("Usage: java -jar metadata-extractor-%s.jar <filename> [<filename>] [-thumb] [-markdown] [-hex]", implementationVersion));
            System.exit(1);
        }
        for (String str : arrayList) {
            ...
            if (!zRemove && arrayList.size() > i2) {
                System.out.printf("%n***** PROCESSING: %s%n%n", str);
            }
            try {
    ...
    public static Metadata readMetadata(InputStream inputStream, long j2) throws ImageProcessingException, IOException {
        BufferedInputStream bufferedInputStream = inputStream instanceof BufferedInputStream ? (BufferedInputStream) inputStream : new BufferedInputStream(inputStream);
        FileType fileTypeDetectFileType = FileTypeDetector.detectFileType(bufferedInputStream);
        Metadata metadata = readMetadata(bufferedInputStream, j2, fileTypeDetectFileType);
        metadata.addDirectory(new FileTypeDirectory(fileTypeDetectFileType));
        return metadata;
    }

    public static Metadata readMetadata(InputStream inputStream, long j2, FileType fileType) throws ImageProcessingException, IOException {
        switch (AnonymousClass1.$SwitchMap$com$drew$imaging$FileType[fileType.ordinal()]) {
            ...
            case 6:
            case 7:
                return TiffMetadataReader.readMetadata(new RandomAccessStreamReader(inputStream, 2048, j2));
            case 8:
