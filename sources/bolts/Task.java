...
public class Task<TResult> {
    ...
    public static Task<Void> delay(long j2) {
        return delay(j2, BoltsExecutors.scheduled(), null);
    }

    public static Task<Void> delay(long j2, CancellationToken cancellationToken) {
        return delay(j2, BoltsExecutors.scheduled(), cancellationToken);
    }

    static Task<Void> delay(long j2, ScheduledExecutorService scheduledExecutorService, CancellationToken cancellationToken) {
        if (cancellationToken != null && cancellationToken.isCancellationRequested()) {
            return cancelled();
        }
        if (j2 <= 0) {
