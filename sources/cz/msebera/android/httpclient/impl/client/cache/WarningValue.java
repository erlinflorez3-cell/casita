...
class WarningValue {
    ...
    public static WarningValue[] getWarningValues(Header header) {
        ...
        while (i2 < value.length()) {
            try {
                ...
                i2 = warningValue.offs;
            } catch (IllegalArgumentException unused) {
                int iIndexOf = value.indexOf(44, i2);
                if (iIndexOf == -1) {
    ...
    protected void consumeQuotedString() {
        ...
        this.offs++;
        boolean z2 = false;
        while (this.offs < this.src.length() && !z2) {
            char cCharAt = this.src.charAt(this.offs);
    ...
    protected void consumeWarnAgent() {
        ...
        try {
            consumeHostPort();
            this.warnAgent = this.src.substring(i2, this.offs);
    ...
    protected void consumeWarnCode() {
        ...
        String str = this.src;
        int i2 = this.offs;
        this.warnCode = Integer.parseInt(str.substring(i2, i2 + 3));
        this.offs += 4;
    }

    protected void consumeWarnDate() {
        int i2 = this.offs;
        Matcher matcher = WARN_DATE_PATTERN.matcher(this.src.substring(i2));
        if (!matcher.lookingAt()) {
            parseError();
        }
        int iEnd = this.offs + matcher.end();
        this.offs = iEnd;
        this.warnDate = DateUtils.parseDate(this.src.substring(i2 + 1, iEnd - 1));
    }

    protected void consumeWarnText() {
        int i2 = this.offs;
        consumeQuotedString();
        this.warnText = this.src.substring(i2, this.offs);
    }
