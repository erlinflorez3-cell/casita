...
class CachedResponseSuitabilityChecker {
        ...
        if (headers != null) {
            for (Header header : headers) {
                for (HeaderElement headerElement : header.getElements()) {
                    ...
                    if ((ProxyConfig.MATCH_ALL_SCHEMES.equals(string) && value != null) || string.equals(value)) {
                        return true;
                    }
                }
            }
    ...
    public boolean allConditionalsMatch(HttpRequest httpRequest, HttpCacheEntry httpCacheEntry, Date date) {
        ...
        boolean z2 = zHasSupportedEtagValidator && etagValidatorMatches(httpRequest, httpCacheEntry);
        boolean z3 = zHasSupportedLastModifiedValidator && lastModifiedValidatorMatches(httpRequest, httpCacheEntry, date);
        if (zHasSupportedEtagValidator && zHasSupportedLastModifiedValidator && (!z2 || !z3)) {
            return false;
        }
        if (!zHasSupportedEtagValidator || z2) {
            return !zHasSupportedLastModifiedValidator || z3;
        }
        return false;
    }
    ...
    public boolean canCachedResponseBeUsed(HttpHost httpHost, HttpRequest httpRequest, HttpCacheEntry httpCacheEntry, Date date) {
        ...
        for (Header header : httpRequest.getHeaders("Cache-Control")) {
            ...
            for (?? r2 = z2; r2 < length; r2++) {
                HeaderElement headerElement = elements[r2];
                ...
                if (HeaderConstants.CACHE_CONTROL_MIN_FRESH.equals(headerElement.getName())) {
                    try {
                        ...
                        if (this.validityStrategy.getFreshnessLifetimeSecs(httpCacheEntry) - this.validityStrategy.getCurrentAgeSecs(httpCacheEntry, date) < j2) {
