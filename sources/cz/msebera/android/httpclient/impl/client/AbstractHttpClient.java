...
public abstract class AbstractHttpClient extends CloseableHttpClient {
    ...
    protected final CloseableHttpResponse doExecute(HttpHost httpHost, HttpRequest httpRequest, HttpContext httpContext) throws IOException {
        HttpContext httpContextCreateHttpContext;
        ...
        synchronized (this) {
            httpContextCreateHttpContext = createHttpContext();
            if (httpContext != null) {
                httpContextCreateHttpContext = new DefaultedHttpContext(httpContext, httpContextCreateHttpContext);
            }
            HttpParams httpParamsDetermineParams = determineParams(httpRequest);
            httpContextCreateHttpContext.setAttribute("http.request-config", HttpClientParamConfig.getRequestConfig(httpParamsDetermineParams));
        ...
        try {
            if (connectionBackoffStrategy == null || backoffManager == null) {
                return CloseableHttpResponseProxy.newProxy(requestDirectorCreateClientRequestDirector.execute(httpHost, httpRequest, httpContextCreateHttpContext));
            }
            HttpRoute httpRouteDetermineRoute = routePlanner.determineRoute(httpHost != null ? httpHost : (HttpHost) determineParams(httpRequest).getParameter(ClientPNames.DEFAULT_HOST), httpRequest, httpContextCreateHttpContext);
            try {
                CloseableHttpResponse closeableHttpResponseNewProxy = CloseableHttpResponseProxy.newProxy(requestDirectorCreateClientRequestDirector.execute(httpHost, httpRequest, httpContextCreateHttpContext));
